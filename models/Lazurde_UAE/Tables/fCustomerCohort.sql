...
SELECT format_date("%B,%Y",customer_acq_date) AS AcquisitionDate,
  ...
  from `noted-computing-279322.halo_1_1_lazurdeEgypt.fOrders` a
  Inner join
  (
    select (lower(user_id)) cus_id, min(order_date) customer_acq_date
    from `noted-computing-279322.halo_1_1_lazurdeEgypt.fOrders` where Is_Successful_Order = true and  order_date!=current_date()
    group by cus_id
  )b
