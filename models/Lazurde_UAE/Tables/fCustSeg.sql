...
`noted-computing-279322.halo_1_1_lazurdeUAE.fCustSegments`)x
left join
(select user_id, orders, revenue from
`noted-computing-279322.halo_1_1_lazurdeUAE.fCustInsights` )y
on x.user_id = y.user_id)a
left join
(select user_id, cast(channel_id as string) channel_id
from `noted-computing-279322.halo_1_1_lazurdeUAE.fCustChannel` where channel_rank =1)b
on a.user_id = b.user_id)c
left join
(select user_id, cast(disc_cat_id as string) disc_cat_id
from `noted-computing-279322.halo_1_1_lazurdeUAE.fCustDisccategory` where disc_cat_rank =1)d
on c.user_id = d.user_id)e
left join
(select user_id, category_L1_id
from `noted-computing-279322.halo_1_1_lazurdeUAE.fCustProductcategory` where Productcategory_rank =1)f
