...
cat.sub_category as category_L1,'' as category_L1_id
from(
...
(
select value_id, attribute_id, entity_id, cast(value as string) value from `noted-computing-279322.halo_1_1_lazurdeEgypt.magento_productentityvarchar`
...
`noted-computing-279322.halo_1_1_lazurdeEgypt.magento_productentityint` atr

left join

`noted-computing-279322.halo_1_1_lazurdeEgypt.magento_producteavoption` op
on atr.attribute_id = op.attribute_id
and atr.value = op.option_id
left join
(select * from `noted-computing-279322.halo_1_1_lazurdeEgypt.magento_producteavoptionvalue` opval

where value_id = (select max(value_id)

from `noted-computing-279322.halo_1_1_lazurdeEgypt.magento_producteavoptionvalue` opval1 where
...
from `noted-computing-279322.halo_1_1_lazurdeEgypt.magento_producteav`
...
(select distinct parent_id,child_id from `noted-computing-279322.halo_1_1_lazurdeEgypt.magento_productrelation` ) par
on base1.variantid=par.child_id
) ))base2

left join

(select entity_id,sku as parent_sku from `noted-computing-279322.halo_1_1_lazurdeEgypt.magento_productentity`) parsku
...
(select entity_id,value from `noted-computing-279322.halo_1_1_lazurdeEgypt.magento_productentitytext`
where attribute_id=374
)color_id

left join

(select max(value) as value,option_id from `noted-computing-279322.halo_1_1_lazurdeEgypt.magento_producteavoptionvalue`
...
(select min(date(created_at)) as launch,product_id as product_id1 from `noted-computing-279322.halo_1_1_lazurdeEgypt.magento_transaction_details`
...
from `noted-computing-279322.halo_1_1_lazurdeEgypt.magento_transaction_details`
) b
...
left join
 ...
 `noted-computing-279322.halo_1_1_lazurdeEgypt.magento_missl_productscat`  cat
...
FROM
`noted-computing-279322.halo_1_1_lazurdeEgypt.magento_productentityint` catalog_product_entity_int
     INNER JOIN
`noted-computing-279322.halo_1_1_lazurdeEgypt.magento_producteavoptionvalue`  eav_attribute_option_value
