...
from `noted-computing-279322.halo_1_1_lazurdeEgypt.fOrders` as orders) ord
left join
...
left join
(Select quantity, cast(product_id as INT64) as product_id, notify_stock_qty from `noted-computing-279322.halo_1_1_lazurdeEgypt.magento_stock_status`) inv
on deli.product_id = inv.product_id
) topsell
left join
(select * from `noted-computing-279322.halo_1_1_lazurdeEgypt.magento_product_index_price`) price
on topsell.product_id = price.entity_id
left join
(select created_at,entity_id from `noted-computing-279322.halo_1_1_lazurdeEgypt.magento_productentity`) live_dt
