...
select *,(cast(M_ga_sessions as float64)*cast(M_ga_bounceRate as float64))/100 Bounces from `noted-computing-279322.halo_1_1_lazurdeEgypt.fGABaseVisitors`
)
group by date_start) visitors

left join

(select count(distinct userid) registered_users,date(activateddate) activateddate  from `noted-computing-279322.halo_1_1_lazurdeEgypt.refUsers`
...
(select sum(cast(M_ga_users as int64)) as new_visitors,startdate date_start from `noted-computing-279322.halo_1_1_lazurdeEgypt.fGABaseVisitors`
