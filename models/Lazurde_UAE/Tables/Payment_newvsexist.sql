...
select distinct entity_id,customer_email,date(datetime(created_at,'Asia/Riyadh')) date
 from `noted-computing-279322.halo_1_1_lazurdeUAE.magento_transaction`
))
--where ranki=1
)a
left join
(select distinct parent_id,method from `noted-computing-279322.halo_1_1_lazurdeUAE.magento_payment` )b
