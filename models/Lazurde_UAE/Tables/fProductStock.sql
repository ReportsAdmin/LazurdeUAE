select Inventory,product_id as productid,null Sizes,Selling_Price,null Discount,
     Case when selling_price>=100 and selling_price<10000 then "0 to 11k"
          ...
          ,MRP
from(
select distinct prd.product_id, prd.stock as inventory, max(pei.value) as MRP, max(pei.value) as Selling_Price from `noted-computing-279322.halo_1_1_lazurdeUAE.fproducts` prd
