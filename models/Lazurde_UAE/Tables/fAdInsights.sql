...
from
    (select cast(date_start as date) date_start,ad_name,adset_name,campaign_name,
     ...
     from `noted-computing-279322.halo_1_1_lazurdeEgypt.fFBBaseTable`
      group by 1,2,3,4) ins,

    (select * except(row_number) from(
     select *,row_number() over (partition by keyword, ad_content, campaign_name) row_number from `noted-computing-279322.halo_1_1_lazurdeEgypt.refKeywords`)
...
from
 ...
 `noted-computing-279322.halo_1_1_lazurdeEgypt.refKeywords` k
